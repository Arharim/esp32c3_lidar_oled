...
public:
    ...
    bool init() {
        ...
        reset();
        if(init_vl53l0x(&vl53l0x_dev) != VL53L0X_ERROR_NONE) return false;
    ...
    bool reset() {
        if(!softwareReset()) return hardwareReset();
        return true;
    }
    bool hardwareReset() {
        if(gpio_xshut == GPIO_NUM_MAX) return false;
   ...
   */
    ...
    bool readSingleWithPolling(uint16_t* pRangeMilliMeter) {
        ...
        VL53L0X_Error status = VL53L0X_PerformSingleRangingMeasurement(
            &vl53l0x_dev, &MeasurementData);
        if(status != VL53L0X_ERROR_NONE) {
            print_pal_error(status, "VL53L0X_PerformSingleRangingMeasurement");
            return false;
        }
        *pRangeMilliMeter = MeasurementData.RangeMilliMeter;
        if(MeasurementData.RangeStatus != 0) return false;
        return true;
    }
    bool readSingleWithDelay(uint16_t* pRangeMilliMeter) {
        ...
        VL53L0X_RangingMeasurementData_t MeasurementData;
        status =
            VL53L0X_GetRangingMeasurementData(&vl53l0x_dev, &MeasurementData);
        if(status != VL53L0X_ERROR_NONE) {
    ...
    bool readSingleWithInterrupt(uint16_t* pRangeMilliMeter) {
        if(gpio_gpio1 == GPIO_NUM_MAX) return false;
        VL53L0X_Error status;
        // set mode
        status = VL53L0X_SetDeviceMode(
            &vl53l0x_dev, VL53L0X_DEVICEMODE_SINGLE_RANGING);
    ...
    void i2cMasterInit(
        ...
        gpio_num_t pin_scl = GPIO_NUM_20,
...
protected:
    ...
    static VL53L0X_Error
        print_pal_error(VL53L0X_Error status, const char* method) {
    ...
    static VL53L0X_Error init_vl53l0x(VL53L0X_Dev_t* pDevice) {
        ...
        status = VL53L0X_PerformRefSpadManagement(
            pDevice, &refSpadCount, &isApertureSpads);
        ESP_LOGI(
            TAG,
            "refSpadCount = %" PRIu32 ", isApertureSpads = %" PRIu8 "\n",
            refSpadCount,
            isApertureSpads);
        if(status != VL53L0X_ERROR_NONE)
