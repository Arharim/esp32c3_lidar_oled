...
int32_t VL53L0X_read_multi(uint8_t address, uint8_t index, uint8_t *pdata,
                           int32_t count);
 ...
 *
 * Manages the big-endian nature of the device (first byte written is the MS
 * byte). Uses SystemVerilog Write Multi task.
 *
 * @code
 *
 * Example:
 ...
 * VL53L0X_get_timestamp_frequency())
 *
